...
functionDeclaration returns [FunctionDeclaration functionDeclarationRet]: //TODO:construct functionDeclaration node
    ...
    Identifier funcName = new Identifier($id.text);
    funcName.setLine($id.line);
    $functionDeclarationRet.setFunctionName(funcName);
    }
...
patternMatching returns [PatternDeclaration patternRet]://TODO:cunstruct patterDeclaration node
    ...
    Identifier pattName = new Identifier($patternName.text);
    pattName.setLine($patternName.line);
    Identifier targetName = new Identifier($targetVar.text);
    targetName.setLine($targetVar.line);
    $patternRet = new PatternDeclaration(pattName, targetName);
...
forStatement returns [ForStatement forStRet]://TODO:construct forStatement node
    f = FOR id = IDENTIFIER
    IN r = range {$forStRet.setRangeExpressions($r.rangeRet);}
    l = loopBody {
    Identifier forName = new Identifier($id.text);
    forName.setLine($id.line);
    $forStRet = new ForStatement(forName, $r.rangeRet, $l.loopExps, $l.loopStmts, $l.loopRetStmt);
...
matchPatternStatement returns [MatchPatternStatement matchPatRet]://TODO:construct match pattern node
    ...
    Identifier matchName = new Identifier($id.text);
    matchName.setLine($id.line);
    $matchPatRet = new MatchPatternStatement(matchName, $e.expRet);
...
expression returns [Expression expRet]:
    ...
    AppendExpression a = (AppendExpression)$expRet;
